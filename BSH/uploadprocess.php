...
function displayDupData($sheetData,$subj_id,$unitNo)
{
    ...
    foreach ($sheetData as $row) {
        ...
        $sql = "SELECT que_desc FROM questions where sub_id=$subj_id and unit_no=$unitNo";
...
function storeInDatabase($sheetData,$subj_id,$unitNo) {
    $servername = "localhost";
$username = "root";
$password = "vikas9652021978";
$database = "gmrit";
$con = mysqli_connect($servername,$username,$password,$database);
    ...
    foreach ($sheetData as $row) {
        ...
        $column3 = mysqli_real_escape_string($con, $row['A']);
        $column4 = mysqli_real_escape_string($con, $row['B']);
        $column5 = mysqli_real_escape_string($con, $row['C']);
        ...
        $sql = "INSERT INTO  questions(que_desc,sub_id,que_mode,que_marks,unit_no) VALUES ('$column3',$subj_id_value,$column5,$column4,$unitNoValue)";
...
if(isset($_POST['similar']))
{
    ...
    if ($_FILES['que']['name']) {
        ...
        if ($filename[1] == 'xls' || $filename[1] == 'xlsx') {
            ...
            displaydupData($sheetData,$subj_id,$unitNo);
...
{   
    ...
    if ($_FILES['que']['name']) {
        ...
        if ($filename[1] == 'xls' || $filename[1] == 'xlsx') {
            ...
            storeInDatabase($sheetData,$subj_id, $unitNo);
        } else {
            echo "Invalid file format. Please upload an Excel file.";
        }
    } else {
